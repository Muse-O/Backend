...
2. passport 전략 실행 후  콜백 라우터에서 프론트도메인으로 redirect 시 쿠키값 안담기는 현상
### 문제
- 프론트와 백 도메인이 달라 passport 전략 실행 후 콜백 라우터에서 서버자체토큰을 발급해서 쿠키값에 담아 프론트주소로 redirect하면 쿠키값이 안담기는 현상 발생
### 해결방안
I. 쿼리스트링에 토큰값을 담아 프론트에 보낸다. 쿼리스트링으로 보내면 보안관련한 이슈가 있을 수 있지만 레디스에 토큰값을 임시저장하고 token_key만 쿼리스트링으로 보낸다면 극복가능하다 판단
II. http-proxy-middleware로 프록시를 설정해서 cors 정책을 우회
### 2번 선택 이유
...
+ 유저 피드백 과정에서 발생된 문제
[유저 피드백 내용]
![image](https://github.com/Muse-O/Backend/assets/51357635/97dd3a28-46e7-469d-a52f-518c791fc935)
![image](https://github.com/Muse-O/Backend/assets/51357635/b3bab5f2-7259-4d98-8251-1ba6a220838e)
...
5. 메시지 HTTPS 적용 후 소켓 통신 장애 - 임건
### 문제
- 클라이언트에서 기존 HTTP 프로토콜을 사용한 웹소캣 방식에서 HTTPS 적용 시 웹소켓 커넥션이 이뤄지지 않는 문제 발생.
![image](https://github.com/Muse-O/Backend/assets/51357635/dde16cac-831c-45e6-a644-bb65422b24a8)
### 해결
- 다음과 같이 Nginx서버의 default파일을 수정. **Upgrade**와 **Connection** 헤더를 추가하여 HTTPS프로토콜에서도 웹소켓 요청을 지원하도록 설정하였습니다.
```js
location /socket.io/ {
