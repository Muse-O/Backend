...
- 전시회 승인요청 리스트
  ...
  &rarr;  전시회를 생성시에 바로 조회가능한 상태가 아니고  
  한번 확인 후 거친후 조회가 되도록 코드값을 ES05로 기본값을 주고  
  기본값으로 설정된 전시회리스트를 조회함.

- 전시회 승인

  &rarr; ES05로 설정되있는 전시회를 확인후 ES01로 변경해주어  
  조회가 되도록 만들어줌.

- 신고

  &rarr; 악성유저, 글, 댓글, 리뷰등을 신고하는 용도  
  신고하게되면 articleReport테이블에 데이터가 저장됌

- 신고 리스트

  &rarr; articleReport테이블에 데이터가 존재할 경우에 조회함.  

- 신고 반영

  &rarr; 리스트에서 신고내용과 해당 게시글의 문제를 확인후  
  신고를 반영해준다. 반영하게되면 해당 게시글이 조회되지않도록 만들어줌.

- 작가 승인 대기자 리스트

  &rarr; UR04(작가 승인대기) 상태인 User 목록과 승인 대기 상태로 변경된 일자(updateAt) 조회

- 작가 권한 승인

  &rarr; UR04(작가 승인대기)상태 인 사용자를 UR02(작가)로 변경
