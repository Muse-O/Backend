...
class NotiRepository {
  ...
  createStream = async (userEmail) => {
    ...
    try {
      const res1 = await this.redis.xgroup(
        "CREATE",
        streamKey,
        consumerGroup,
        "$",
      ...
      console.log(
        `Consumer Group ${consumerGroup} created on Stream ${streamKey}`
      );
    } catch (error) {
  ...
  saveToStream = async (userEmail, notiData) => {
    ...
    try {
      const res = await this.redis.xadd(
        streamKey,
        "*",
      ...
      console.log(
        `Assigned to consumer, Stream data: ${JSON.stringify(streamData)}`
      );
    } catch (error) {
      logger.error(error.message);
    }
  ...
  deleteStream = async (userEmail) => {
    ...
    try {
      ...
      console.log(
        `Consumer Group ${consumerGroup} destroyed on Stream ${streamKey}`
      );
